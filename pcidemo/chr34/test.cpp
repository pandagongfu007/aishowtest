...
void CardChIntRxCfg();//通道中断接收配置
void CardChIntRxOp();//中断接收操作
...
void InitCard()
{
	...
	CHR34XXX_GetDevParInfo(devId, &stDevParInfo);//设备参数信息结构
...
{
	//通道模式设置
	CHR34XXX_Ch_SetMode(devId, testCh0, 2);//设置为rs485 模式Mode：通道模式，0 表示 232 模式，1 表示 422 模式，2 表示 485 模式
										   //配置串口(波特率等)
	...
	CHR34XXX_Ch_SetType(devId, testCh0, 1);//type串口类型 0:同步串口  1:异步串口
	CHR34XXX_Asyn_RxCh_SetMode(devId, testCh0, 0);//mode接收模式 0:透明接收  1:协议接收（仅适用于异步串口）
}
...
void CardChRxCfg()
{
	...
	CHR34XXX_RxCh_ClearOFFlag(devId, testCh0);//清空缓冲区溢出标记
											  //设置接收中断模式
	CHR34XXX_RxInt_Enable(devId, testCh0, FALSE);//禁止中断 Enabled:0 禁止, 1 使能 
												 //接收中断触发深度
	CHR34XXX_Asyn_RxCh_IntDepth(devId, testCh0, 10);//depth:接收中断深度
													//复位接收FIFO
	CHR34XXX_RxCh_ClearFIFO(devId, testCh0);
	//开始接收
	CHR34XXX_RxCh_Start(devId, testCh0);
}
...
void CardChIntRxCfg()
{
	...
	CHR34XXX_Asyn_RxCh_SetMode(devId, testCh0, 0);  //接收模式mode 0:透明接收  1:协议接收（仅适用于普通串口）
	CHR34XXX_RxInt_Enable(devId, testCh0, 0);   //接收中断使能Enabled 0:不使能中断接收模式 1：使能中断接收模式
	CHR34XXX_Asyn_RxCh_IntDepth(devId, testCh0, 0); //depth:接收中断深度
...
void CardChIntRxOp()
{
	...
	DWORD Returned = 0;//实际接收的数据量
					   //等待接收中断事件
	DWORD intRes = CHR34XXX_RxInt_WaitEvent(devId, hEvent, 10);
	for (int i = 0;i<MAXCHNUM;i++)
	{
		if ((intRes >> i) & 1)
		{
			//读取接收FIFO数据量
			DWORD count = 0;
			CHR34XXX_Asyn_RxCh_FIFOCount(devId, i, &count);//Count：接收缓冲区数据量
														   //若接收FIFO的数据量不为0，则可读取数据
			if (count > 0)
			{
				//读接收fifo数据
				CHR34XXX_Asyn_RxCh_Read(devId, i, count, Buffer, &Returned);//count：待读取的数据量 Buffer：存放接收到的数据 Returned：实际读取到的数据量
																			//打印接收到的数据
...
{
	//设置发送模式
	if (CHR34XXX_TxCh_SetMode(devId, testCh0, 0) == FALSE)//Mode：通道模式，0 表示 232 模式，1 表示 422 模式，2 表示 485 模式
	{
