...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while(!kbhit())
	{
		...
		CHR34xxx_RecvData(ChNum,m_recvCfg.RecvIntEn,m_recvCfg.ASYN_RecvMode,ChWorkMode,m_ptCGF.PtRxMode,Rxbuf,&dwRxResult);
		//打印接收数据
		for (int i=0;i<dwRxResult;i++)
		{
			printf("RevData = %x\n",Rxbuf[i]);
		}
		Sleep(1000);
...
void initApiParam()
{
	...
	m_recvCfg.IntDeepth = 10;//中断深度
	m_recvCfg.RecvIntEn = 1;//接收中断使能(0:禁止中断,1:中断使能)
